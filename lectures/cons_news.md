...
- how the Kalman filter introduced in [this lecture](https://python-intro.quantecon.org/kalman.html)
  ...
  forecasting introduced in {doc}`this lecture <tools:classical_filtering>`
  embody lessons
...
- a simple application of alternative ways to factor a covariance
  generating function along lines described in {doc}`this lecture <tools:classical_filtering>`
...
Using calculations in the {doc}`quantecon lecture <tools:classical_filtering>`, where
...
described in {doc}`this lecture <tools:classical_filtering>`.
